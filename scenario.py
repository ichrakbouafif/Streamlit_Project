    ...
    for annee_data in recap_data:
        with st.expander(f"Détails de calcul {ratio_type} pour {annee_data['Année']}"):
            ...
            if ratio_type == "LCR":
                ...
                with col2:
                    st.metric("HQLA", f"{annee_data['HQLA']:,.2f}")
                with col3:
                    st.metric("Outflows", f"{annee_data['Outflows']:,.2f}")
                with col4:
                    st.metric("Inflows", f"{annee_data['Inflows']:,.2f}")
                ...
                with col2:
                    st.metric("ASF", f"{annee_data['ASF']:,.2f}")
                with col3:
                    st.metric("RSF", f"{annee_data['RSF']:,.2f}")
