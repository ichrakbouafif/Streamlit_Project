...
def show():
    ...
    st.write(f"L'horizon de stress test est de {horizon} années.")

    # Dictionnaire des ratios à calculer avec les composants spécifiques comme numérateur et dénominateur
    ratios = {
        "LCR": {
            ...
            "baseline_value": 120,  # Valeur de base statique pour LCR
            "projected_values": [115, 110, 105, 130, 150],  # Valeurs projetées pour LCR sur 3 années
            "numerator": [150, 145, 140, 200, 320],  # Valeur fictive du numérateur pour LCR (Actifs liquides)
            "denominator": [130, 132, 135, 140, 160]  # Valeur fictive du dénominateur pour LCR (Sorties nettes de trésorerie)
        },
        "NSFR": {
            ...
            "column": [
                ...
                "Besoins en financements stables"
            ],
            ...
            "baseline_value": 105,  # Valeur de base statique pour NSFR
            "projected_values": [102, 100, 98, 70, 40],  # Valeurs projetées pour NSFR sur 3 années
            "numerator": [200, 190, 180, 140, 150],  # Valeur fictive du numérateur pour NSFR (Financements stables)
            "denominator": [190, 188, 185, 145, 180]  # Valeur fictive du dénominateur pour NSFR (Besoins en financements stables)
        },
        "Levier": {
            ...
            "baseline_value": 0.1,  # Valeur de base statique pour Levier
            "projected_values": [0.11, 0.13, 0.16, 0.12, 0.17],  # Valeurs projetées pour Levier sur 3 années
            "numerator": [10, 12, 14, 17, 19],  # Valeur fictive du numérateur pour Levier (Fonds propres de base)
            "denominator": [100, 92, 88, 97, 99]  # Valeur fictive du dénominateur pour Levier (Total des expositions)
        },
        "Solvabilité": {
            ...
            "baseline_value": 12,  # Valeur de base statique pour Solvabilité
            "projected_values": [11.8, 11.5, 11.2, 15.5, 13],  # Valeurs projetées pour Solvabilité sur 3 années
            "numerator": [120, 118, 115, 150, 120],  # Valeur fictive du numérateur pour Solvabilité (Fonds propres réglementaires)
            "denominator": [1000, 1025, 1030, 1200, 1450]  # Valeur fictive du dénominateur pour Solvabilité (Risques pondérés)
        }
    }
    ...
    if st.button("Calculer les Ratios"):
        ...
        for ratio, details in ratios.items():
            ...
            # Afficher les composants avec des bullet points et retour à la ligne
            st.write("**Composants :**")
            for composant in details["composants"]:
                st.write(f"- {composant}")

            # Afficher l'interprétation
            st.write(f"**Interprétation :** {details['interpretation']}")

            # Valeur de base, valeurs projetées et valeurs des numérateurs et dénominateurs
            baseline_value = details["baseline_value"]
            projected_values = details["projected_values"]
            numerators = details["numerator"]
            denominators = details["denominator"]

            # Table pour afficher les valeurs, numérateurs et dénominateurs
            ratio_table = pd.DataFrame({
                "Valeur": ["Valeur de base"] + [f"Valeur projetée Année {i+1}" for i in range(horizon)],
                "Valeurs": [baseline_value] + projected_values[:horizon],
                details["column"][0]: [numerators[0]] + numerators[:horizon],  # Numérateur (ex: Actifs liquides de haute qualité)
                details["column"][1]: [denominators[0]] + denominators[:horizon]  # Dénominateur (ex: Sorties nettes de trésorerie)
            })
            st.table(ratio_table)

            # Vérification si le ratio est conforme (selon les règles des ratios)
            ...
            # Ajouter les résultats à un tableau récapitulatif
            result_data.append({
                "Ratio": ratio,
        ...
        # Tableau récapitulatif avec la conformité en couleur
        result_df = pd.DataFrame(result_data)

        # Appliquer la couleur pour la colonne conformité (vert pour conforme, rouge pour non conforme)
        def apply_color(val):
        ...
        st.markdown(styled_df.to_html(escape=False, index=True), unsafe_allow_html=True)

    # Ajouter un bouton pour procéder aux choix du scénario
    if st.button("Procéder aux choix du scénario"):
            # Set session state to navigate to the "Choix du Scénario" page
